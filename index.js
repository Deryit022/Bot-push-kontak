...
async function startSesi() {
    ...
    const x = require("./mts4you").makeWASocket(connectionUpdate)
    ...
    x.ev.on("connection.update", ({ connection }) => {
      if (connection === "open") {
        console.log("KONEKSI YawwOffc " + "Terhubung (" + x.user?.["id"]["split"](":")[0] + ")")
      }
      if (connection === "close") {
        startSesi()
      }
      if (connection === "connecting") {
        if (x.user) {
          console.log("SC YawwOffc\nYT : @nuradistore\nCREATOR NURADI\nSELAMAT MENIKMATI")
        } else if (!useQR && !useCODE) {
          console.log("YawwOffc " + "Autentikasi Dibutuhkan\nGunakan Perintah \x1B[36mnpm start\x1B[0m untuk terhubung menggunakan nomor telepon\n\n\x1B[1m\x1B[41m Full Tutorial Check di Youtube: @nuradistorexjb \x1B[0m\n\n")
        }
      }
    })
      ...
      if (isWelcome) {
        ...
        for (let num of participants) {
          try {
            ...
            ppuser = 'https://telegra.ph/file/241b747767455c4bcfc7b.jpg'
          }
          if (anu.action == 'add') {
