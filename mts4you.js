...
let anu = groups.map((v) => v.id)
  ...
  x.pushContacts = async (nsgegs, idgcnye, txt1ortxt2) => {
    ...
    x.reply(nsgegs, "BOT BY YawwOffc\nPUSH CONTACTS\n" + conError)
    }
  }
  ...
  x.sendChatAllGroup = async (mesegesp, tx1tx2) => {
    ...
    if (!Txt1 || !Txt2 || isNaN(Txt2)) {
      return x.reply(mesegesp, "BOT BY YawwOffc\nCHAT ALL GROUP\n\n" + "*Format yang anda berikan tidak valid*\n*Contoh*: .jpm Hallo ... Pesan|5*")
    } else {
      await x.reply(mesegesp, "BOT BY YawwOffc\nCHAT ALL GROUP\n\n" + "*Broadcast Start*:\n*Target*: " + anu.length + " groups\n*Text*: " + Txt1 + "\n*Delay*: " + Txt2)
      let idgcwy = 0
      const stIntvral = setInterval(async () => {
    ...
    x.reply(mesegesp, "BOT BY YawwOffc\nCHAT ALL GROUP\n\n" + ErrorK)
    }
  }
  ...
  x.saveContacts = async (messgs, Prtcipnts) => {
    ...
    x.reply(messgs, "BOT BY YawwOffc\nAUTO SAVE CONTACTS\n\n" + '' + SvdError)
    }
  }
